...
function parseValue(raw: string | undefined): ParseResult {
  ...
  try {
    ...
    const categories = parsed.map(category => {
      ...
      const rawItems = Array.isArray(categoryObj.tools_list)
        ? categoryObj.tools_list
...
function serializeValue(categories: InternalCategory[]): string {
  const normalized = categories.map(category => ({
    ...
    tools_list: category.items.map(item => ({
...
export function ToolListEditor({ value, onValueChange }: ToolListEditorProps) {
  ...
  const updateCategoryDescription = React.useCallback(
    (id: string, description: string) => {
      updateCategories(categories.map(c => (c._id === id ? { ...c, data: { ...c.data, description } } : c)));
    },
    [categories, updateCategories]
  );
          ...
          c._id === categoryId
            ? {
                ...
                items: [
                  ...
                  {
                    _id: nextId(),
                    data: { name: "", image: "", link: "", description: "" },
                  },
                ],
              }
            : c
  ...
  if (hasParseError) {
    return (
      <div className="rounded-xl border border-warning/30 bg-warning/5 px-4 py-3 text-sm text-warning-700 dark:text-warning-400">
        配置格式无效，请确认为数组结构（支持 title/name、tools_list/items 等字段）。
      </div>
    );
  }
      ...
      <Reorder.Group axis="y" values={categories} onReorder={updateCategories} className="space-y-2">
        {categories.map((cat, idx) => (
          <CategoryCard
            ...
            onDescriptionChange={v => updateCategoryDescription(cat._id, v)}
    ...
    >
      ...
      >
        <div className="border-t border-border/40 px-3 py-3">
          <Input
            size="sm"
            value={String(category.data.description ?? "")}
            onValueChange={onDescriptionChange}
            placeholder="分类描述（可选）"
            classNames={{
              inputWrapper: cn(inputWrapperClass, "mb-3 min-h-8 h-8"),
              input: "text-sm",
            }}
          />
